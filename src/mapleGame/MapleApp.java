...
//		new MapleApp();
	...
	public void init() {
		...
		mushroom = new Mushroom("image/ÁÖÈ²¹ö¼¸¿À¸¥ÂÊ.gif", 555, 380, 30, "ÁÖÈ²¹ö¼¸");
		stone = new Stone("image/Stone.gif", 100, 150, 30, "½ºÅæ°ñ·½");
		barlog = new Barlog("image/¹ß·Ï¿À¸¥ÂÊ.gif", 200, -20, 30, "ÁÖ´Ï¾î¹ß·Ï");
		block = new Block("image/ºí·Ï°ñ·½¿À¸¥ÂÊ.gif", 200, 300, 30, "ºí·Ï°ñ·½");
		boss = new Boss("image/ÀÚÄñ.gif", 300, 20, 800, "ÀÚÄñ");
				...
				try {
					for (int i = 0; i < enemy.size(); i++) {
						...
						if (enemy.get(i) instanceof Boss && enemy.get(i).hp < 380) {
							enemy.get(i).setIcon(new ImageIcon("image/ÀÚÄñ°³ÇÇ.gif"));
							enemy.get(i).y = enemy.get(i).y + 30;
							enemy.get(i).x = enemy.get(i).x + 50;
						}
						// º¸½º
						if (isBossStart == true) {
							if (deadEnemy[2] > 170) { // ¼³Á¤ÇÑ Á¡¼ö ÀÌ»óÀÌ¸é º¸½º¸Ê ÀÔÀå
								isBoss = true;
							}

							if (isBoss == true) {

								// int result = JOptionPane.showConfirmDialog(null, "º¸½º·ëÀÔÀå!", "¾È³»¸Þ¼¼Áö",
						...
						if (crash(player.x, player.y, enemy.get(i).x, enemy.get(i).y, player.width, player.height,
							...
							if (player.hp <= 0) {
								...
								int result = JOptionPane.showConfirmDialog(null,
										"Á×¾ú³×... Á¡¼ö´Â " + MapleApp.deadEnemy[2] + "Á¡ ÀÔ´Ï´Ù.", "¾È³»¸Þ¼¼Áö",
										JOptionPane.OK_OPTION);
						...
						if (player.isAttack == true) {
							...
							if (attackCrash(player.x, player.y, enemy.get(i).x, enemy.get(i).y, player.width,
								...
								if (player.isAttack == true) {
									...
									if (enemy.get(i).hp <= 0) {
										System.out.println(enemy.get(i).name + " Á×À½...");
										score(enemy.get(i).name); // Á¡¼ö °è»ê
